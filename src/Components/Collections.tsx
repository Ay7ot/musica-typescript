        ...
        {navToggled ? 
          ...
          <section className={`${width > 768 ? 'relative left-[5rem] mt-[2.1rem] w-[85vw] sm:mt-4 ' :'mt-6'}`}>
            <div className='grid grid-cols-2 gap-5 sm:flex sticky top-[65px] bg-[#100e0e] pb-4 pt-[2px]'>
              <button className={`p-2 text-[#949798] text-[14px] border-[1px] rounded-full md:px-10 ${collections.isLikedPlaylistActive ? 'bg-[#FACD66] border-[#FACD66] text-[#1D2123]' : ''}`} onClick={()=>dispatch({type: 'setToLikedPlaylist'})}>My Collections</button>
              <button className={`p-2 text-[#949798] text-[14px] border-[1px] rounded-full md:px-10 ${collections.isLikedSongsActive ? 'bg-[#FACD66] border-[#FACD66] text-[#1D2123]' : ''}`} onClick={()=>dispatch({type: 'setToLikedSongs'})}>My Likes</button>
            </div>
            {
